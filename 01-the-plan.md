# The Plan

## The week

Honest times below. **Hands-on** is how long you're actually doing something; **wall clock**
includes waiting for water to boil and rice to microwave.

| | Lunch | Dinner | Cuisine | Hands-on | Wall clock |
|---|---|---|---|---|---|
| **Mon** | Chana masala bowl | Sticky tofu, rice, roasted veg | Chinese | 10 min | **15 min** |
| **Tue** | Mediterranean bowl | Rigatoni + lentil-walnut ragù | Italian | 6 min | **18 min** |
| **Wed** | Sichuan crumble bowl | Veggie chowmein, egg, edamame | Nepali · Indo-Chinese | 14 min | **18 min** |
| **Thu** | Mediterranean bowl | Chana masala, jeera rice, crumble sabzi | South Asian | 12 min | **15 min** |
| **Fri** | Ragù rice bowl | Tacos, chili-lime crumbles | Mexican | 15 min | **18 min** |

Lunches are assembled on the weekend and reheated at work — they cost you nothing on the day.

**Tuesday's 18 minutes is mostly the water boiling.** Put the pot on the second you walk in
the door, before you change out of your work clothes, and it overlaps with everything else.
An electric kettle cuts it to 12.

**Breakfast, every day:** the post-workout shake — protein + creatine + rolled oats +
frozen banana + ground flax + soy milk. See [`05-nutrition.md`](05-nutrition.md).

---

## Lunches — what's in each container

Five lunches × two people = **ten containers**. Pack Monday through Wednesday on Sunday;
pack Thursday and Friday on Wednesday night so nothing you eat is five days old.

| Day | Container | Packed separately |
|---|---|---|
| **Mon** | Chana masala · rice · shelled edamame · cilantro | Lemon wedge |
| **Tue** | Rice · harissa roasted veg · roasted chickpeas · cucumber · cherry tomato · pickled onion · parsley | **Tahini-lemon sauce** |
| **Wed** | Rice · roasted veg · edamame · cucumber | **Sichuan crumbles** and **gochujang-sesame** |
| **Thu** | Same as Tuesday | **Tahini-lemon sauce** |
| **Fri** | Rice · lentil-walnut ragù · roasted veg | Nutritional yeast |

**Friday's ragù goes over rice, not pasta.** Reheated pasta in a container turns gluey and
sad. Rice takes the same sauce and survives four days.

**Anything crisp or creamy travels separately.** Tahini dressed on Sunday turns grains to
paste by Tuesday. Crumbles packed against wet rice lose everything that makes them good.
Two small containers is worth the extra thirty seconds.

---

## Where the tofu crumbles go

One sheet pan, three zones, roughly **two cups of each**. Every zone has a job:

| Zone | Goes to | Uses |
|---|---|---|
| **Sichuan** — five-spice, Sichuan pepper, chili crisp | Wednesday lunch bowls ×2 · Wednesday dinner chowmein | ~2 cups |
| **Cumin–garam masala** — cumin, coriander, garam masala, turmeric | Thursday dinner sabzi ×2 · scattered on Monday's lunch | ~2 cups |
| **Chili–lime** — smoked paprika, cumin, chipotle, lime | Friday dinner tacos ×2 | ~2 cups |

**On falafel weeks, season two zones, not three.** Friday is falafel night, so nothing eats
the chili–lime. Split the pan between Sichuan and cumin–garam masala instead.

Whatever's left over is the snack. That's not a consolation prize — a bowl of Sichuan
crumbles at 4pm is the reason this component exists.

**And where the crispy tofu goes:** two blocks, roughly 800 g. Monday's dinner takes about
half. The rest is the week's protein booster — four minutes in the air fryer brings it
straight back, and it goes on any bowl that's running short.

**The Mediterranean bowls don't use crumbles.** Their protein is the roasted chickpeas from
the harissa tray, plus tahini and an edamame scoop. That's deliberate — it keeps the
chickpeas and the crumbles from competing for the same slot.

---

## Why it's built this way

**Anchors carry the low-energy load.** The chana masala and the ragù are finished dishes
that need nothing but a microwave. Both taste better on day three than day one.

**The kit carries the variety.** Crispy tofu, three-zone crumbles, rice, two trays of
roasted veg, two sauces, a shredded stir-fry pack, and a bag of frozen edamame.

**Nothing is made "spare."** Every jar, tray and zone below names the meal that eats it. If
something on prep day has no meal attached, don't make it.

**Everything is doubled and half is frozen:**

| | What you do | Time |
|---|---|---|
| **Week A** | Full prep, double batches of both anchors | ~2h45 |
| **Week B** | Thaw anchors. Fresh rice, one veg tray, one sauce, one tray of crumbles | ~1 hr |

Averages out to about **1h55 a week**, against roughly ten hours of weeknight cooking.

---

## Anchor rotation

Fixed slots, different dish each cycle. Same technique, same prep time.

| Cycle | Anchor 1 — South Asian, Instant Pot | Anchor 2 — pasta sauce, stovetop |
|---|---|---|
| 1 | Chana masala | Lentil-walnut ragù |
| 2 | Rajma (kidney bean curry) | Roasted red pepper & white bean |
| 3 | Dal tadka | Chickpea puttanesca |
| 4 | Sambar-style lentil & vegetable | Creamy cashew-tomato with lentils |
| 5 | Aloo chana | Back to lentil-walnut ragù |

With alternating freezer weeks, one pass through this table is **ten weeks** before
anything repeats.

---

## Portioning

You two are roughly 100 lb and 183 lb, so a single portion size can't work. Rather than
cooking two menus, cook one and **boost at the plate.**

| | Person A (~103 lb) | Person B (~183 lb) |
|---|---|---|
| Grain | 1 cup cooked | 1½ cups cooked |
| Anchor / main | 1½ cups | 2 cups |
| Edamame | ½ cup shelled | 1 cup shelled |
| Extra | — | + ¼ cup crumbles, or 100 g extra tofu |

**Booster cheat sheet:**

| Booster | Protein | Calories |
|---|---|---|
| ½ cup shelled edamame | 9 g | 90 |
| 1 cup shelled edamame | 18 g | 180 |
| 100 g extra-firm tofu | 10 g | 145 |
| ¼ cup tofu crumbles | ~8 g | 110 |
| 2 tbsp peanut butter | 8 g | 190 |

---

## Packing notes

- **Cool everything fast** before it goes in the fridge — spread hot food on a sheet pan
  for ten minutes rather than sealing a hot container. Cooked rice especially wants to get
  cold quickly and be eaten within four days.
- **Freeze half the rice** in flat portions. It reheats better from frozen than from four
  days in the fridge.
- **Label with the date.**

---

## Ramp-up

| When | What |
|---|---|
| **Weeks 1–2** | Core system only. No projects. Get the timeline down. |
| **Week 3** | The seitan session — one 2-hour block, ~3 kg in three flavors, frozen in portions. |
| **Week 5+** | Check the Asian grocery's refrigerated case for tempeh. Add one tempeh dinner. |
| **Whenever** | Optional: homemade tempeh, fresh noodles, flatbread. |

### Where the seitan goes

The three flavors aren't three new dinners. **They rotate into slots that already exist** —
that's why those three flavors and not others.

| Flavor | Takes this slot | What changes |
|---|---|---|
| **Char siu** | Monday, alternating with sticky tofu | Same technique. Sear the slices hard in a dry pan, then toss in the sticky glaze off the heat. Rice and chili-cumin veg. |
| **Chorizo** | Friday, alternating with chili-lime crumbles | Straight swap into the tacos. Crumble it into the pan rather than slicing. |
| **Shawarma** | Tuesday & Thursday **lunch** — upgrades the Mediterranean bowl | Tahini, pickled onion and harissa veg are already in that container. Adding sliced shawarma seitan turns a chickpea bowl into a shawarma bowl, and takes it from ~33 g protein to ~50 g. |

**No new shopping, no new sauces, no new techniques.** The kit was built to receive them.

Alternate week to week — seitan Monday one week, sticky tofu the next. Three kilos frozen in
portions covers about a month of that, at which point you run the session again.

The only thing the seitan session changes about your Sunday is that it **removes** work:
a week where seitan takes the Monday slot is a week you don't marinate and air-fry tofu.

### Friday alternates: tacos ↔ falafel

Falafel takes the Friday slot every other week. It's the same assembly pattern as taco
night — warm flatbread, a crisp protein, pickled onions, a creamy sauce, a slaw — so
**three of the five components are already in your kit.**

The trick is freezing it in the right state: **formed raw, cooked fresh.** Reheated falafel
is dry; raw frozen falafel air-fried on the night is excellent. Costs ~15 min on Sunday and
nothing on Friday.

**One hard rule:** dried chickpeas soaked overnight, never cooked or canned. Soak a second
bowl on Saturday alongside the chana ones — these don't get cooked. Full method in
[`recipes/`](recipes/).

Falafel is less protein-dense than tofu or seitan (~19 g a serving), so Person B wants a
full cup of edamame or a scoop of hummus alongside.

### Where the tempeh goes

The obvious first move is a **satay-style tempeh bowl** in the Monday slot: rice, cucumber,
edamame, crushed peanuts, and Monday's usual glaze with 2 tbsp peanut butter and a squeeze
of lime whisked in. That's a sauce made for one meal, on the night, rather than a jar that
sits in the fridge all week.

**Steam the tempeh for 10 minutes before you cook it.** This removes the bitterness, and
skipping it is the single reason most people think they don't like tempeh. Then pan-sear
and glaze as normal.

---

## Protein frequency rules

The point isn't to ban anything — it's to make sure nothing carries the whole week.

| Source | Frequency |
|---|---|
| Tofu (block and crumbled) | 2–3 dinners |
| Lentils, chickpeas, beans | 2 dinners + most lunches |
| Edamame | Daily, as the booster |
| Tempeh | 1 dinner (from week 5) |
| Seitan | 1 dinner (from week 3) |
| Eggs | 2 dishes maximum, both optional |
| Impossible / TVP | **Once a week, maximum** — Friday treat night |
